...
ranked_users AS (
    SELECT user_id, email, role,
           ROW_NUMBER() OVER (PARTITION BY role ORDER BY user_id) AS rank
    FROM inserted_users
);

--- Creating some basic items based on 
INSERT INTO suppliers(supplier_id, validated, request_datetime, collab_type, supplier_since)
VALUES ();